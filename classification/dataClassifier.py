...
min_switch_loop = 4
vert_s_single = "vert_scount_{0}_single"
vert_s_loop = "vert_scount_{0}_loop"
...
def enhancedFeatureExtractorDigit(datum):
    ...
    The enhanced feature is trying to indicate the classifier that there is possibility
    of loop by detecting how many pixels switched the value. In case of a loop, there will
    be more switches while non-loop areas will have less number of switches.
    
    The optimum value for minimum switches required to detect a loop is found heuristically
    by trying out different values from 3 to 7.
    ##
    """
    ...
    for y in range(DIGIT_DATUM_HEIGHT):
        ...
        prevPixval = 0
        for x in range(DIGIT_DATUM_WIDTH):
            if datum.getPixel(x, y) != prevPixval:
                switch_count = switch_count + 1
                prevPixval = datum.getPixel(x, y)
        features[vert_s_single.format(y)]=0
        features[vert_s_loop.format(y)]=0
        if switch_count > 1:
            features[vert_s_single.format(y)]=1
        if switch_count > min_switch_loop:
            features[vert_s_loop.format(y)]=1  
...
foodCountF = "foodCountFeature"
gDistF = "minGhostDistFeature"
cDistF = "minCapsuleDistFeature"
foodDistF = "minFoodDistFeature"

def computeMinDistF(pacPos, fPosList):
    #print "----------------------------------------------------------"
    #print "Inside computeMinDistF",pacPos,fPosList
    minvalue = float('inf')
    if not fPosList:
        minvalue = 0
        return minvalue 
    for fPos in fPosList:
        dist = util.manhattanDistance(pacPos, fPos)
        if dist < minvalue:
            minvalue = dist
    #print "Min Vlaue",minvalue
...
def enhancedPacmanFeatures(state, action):
    ...
    #print "legal actions",state.getLegalActions()
    #print "legal sucessors",state.generatePacmanSuccessor(action)
    #print "food list",state.getFood().asList()
    #print "ghost list",state.getGhostPositions()
    #print "capsule list",state.getCapsules()
    
    # Add Food count feature
    state = state.generatePacmanSuccessor(action)
    features[foodCountF] = state.getNumFood()
    
    features[gDistF] = computeMinDistF(state.getPacmanPosition(),state.getGhostPositions())
    #features[cDistF] = computeMinDistF(state.getPacmanPosition(),state.getCapsules())
    features[foodDistF] = computeMinDistF(state.getPacmanPosition(),state.getFood().asList())
