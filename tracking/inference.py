...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        #print "self belief",self.beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while numberOfParticles > 0:
            ...
            if (posIndex < len(self.legalPositions)-1):
                posIndex = posIndex+1
            else:
                posIndex = 0
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        newWeightedBelief = util.Counter()
        oldBelief = self.getBeliefDistribution()
    
        if(noisyDistance == None):
            self.particles = []
            for numPart in range(self.numParticles):
                self.particles.append(self.getJailPosition())
        else:
            for pos in self.legalPositions:
                trueDistance = util.manhattanDistance(pos, pacmanPosition)
                newWeightedBelief[pos] = emissionModel[trueDistance] * oldBelief[pos]
            newWeightedBelief.normalize() 
        
            #print "Belief:",newWeightedBelief
            if(newWeightedBelief.totalCount() == 0):
                self.initializeUniformly(gameState)
            else:
                self.particles = []
                for numPart in range(self.numParticles):
                    self.particles.append(util.sample(newWeightedBelief))
                #print newWeightedBelief.values()
            #self.particles = util.nSample(newWeightedBelief,newWeightedBelief.values(),total_samples)   
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        newParticles = []
        newTimeElapseDist = util.Counter()
        for particle in self.particles:
            newTimeElapseDist = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            newParticles.append(util.sample(newTimeElapseDist))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.jointGhostsPos = []
        #print("ghost particles:",self.legalPositions)
        #print "Num ghosts",self.numGhosts
        #print "self.numParticles",self.numParticles
        self.jointShuffleGhostsPos = list(itertools.product(self.legalPositions,repeat=self.numGhosts))
        random.shuffle(self.jointShuffleGhostsPos)
        #print "suffleGhostsPos",self.jointShuffleGhostsPos, len(self.jointShuffleGhostsPos)
        
        numberOfParticles = self.numParticles
        self.particles = []
        posIndex = 0
        while numberOfParticles > 0:
            self.particles.append(self.jointShuffleGhostsPos[posIndex])
            if (posIndex < len(self.jointShuffleGhostsPos)-1):
                posIndex = posIndex+1
            else:
                posIndex = 0
            numberOfParticles = numberOfParticles - 1
    ...
    def observeState(self, gameState):
        ...
        #print "Emimodel",emissionModels
        newWeightedBeliefs = util.Counter()
        oldBeliefs = self.getBeliefDistribution()
        #print "oldBeliefs:",oldBeliefs   
        
        for pIndex,sample in enumerate(self.particles):
                for gIndex in range(self.numGhosts):
                    if(noisyDistances[gIndex] == None):
                        self.particles[pIndex] = self.getParticleWithGhostInJail(sample,gIndex)
                        
        for sample in oldBeliefs:
            partM = 1.0
            for gIndex in range(self.numGhosts):
                if(noisyDistances[gIndex] != None):
                    trueDistance = util.manhattanDistance(sample[gIndex], pacmanPosition)
                    partM = partM * emissionModels[gIndex][trueDistance]
                    
            newWeightedBeliefs[sample] = partM * oldBeliefs[sample]       #TODO:We need to return from here
            
        newWeightedBeliefs.normalize()

        if(newWeightedBeliefs.totalCount() == 0):
            self.initializeParticles()
     
        else:
            self.particles = []
            for numPart in range(self.numParticles):
                self.particles.append(util.sample(newWeightedBeliefs))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for gIndex in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle),gIndex,self.ghostAgents[gIndex])
                newParticle[gIndex] = util.sample(newPosDist) 
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        jointBeliefDist = util.Counter()
        for particle in self.particles:
            jointBeliefDist[particle] = jointBeliefDist[particle] + 1
        jointBeliefDist.normalize()
        #print "distribution", jointBeliefDist
        return jointBeliefDist
